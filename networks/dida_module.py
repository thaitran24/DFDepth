...
class DIDABlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, shortcut=False):
        super().__init__()
        # Default channel reduction rate for DIDA Module is m=16
        self.conv1 = DIDAModule(3, inplanes, planes, m=16, padding=None, stride=stride)
...
class DIDABottleneck(nn.Module):
    expansion = 4
